...
#[sea_orm(table_name = "water_model")]
#[crudcrate(
    api_struct = "WaterModel",
    name_singular = "water model",
    name_plural = "water-models",
    generate_router,
    operations = WaterModelOperations,
)]
pub struct Model {
...
pub struct WaterModelOperations;

#[async_trait::async_trait]
impl crudcrate::CRUDOperations for WaterModelOperations {
    type Resource = WaterModel;
    ...
    async fn after_get_one(
        ...
        entity.layer_count =
            Some(crate::common::layer_counts::fetch_layer_count(db, "water_model_id", entity.id).await?);
        Ok(())
    }
    ...
    async fn after_get_all(
        ...
        let map = crate::common::layer_counts::fetch_layer_counts(db, "water_model_id", &ids).await?;
